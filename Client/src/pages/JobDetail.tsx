...
const JobDetail = () => {
  ...
  useEffect(() => {
    if (id) {
      const jobData = getJobPostById(id);
      if (jobData) {
        setJob(jobData);
        setEditForm({
          title: jobData.title,
        ...
        // Get matching applicants if user is the recruiter
        if (user?.userType === "recruiter" && jobData.recruiterId === user.id) {
          const applicants = getMatchingApplicants(id);
          setMatchingApplicants(applicants);
        }
        
        // Check if applicant has already connected for this job
        if (user?.userType === "applicant") {
          const connection = connections.find(
            conn => conn.jobPostId === id && conn.applicantId === user.id
          );
  ...
  }, [id, getJobPostById, user, connections, getMatchingApplicants]);
  ...
  const handleSaveChanges = () => {
    ...
    const updatedJob = {
      ...
      skillsRequired: editForm.skillsRequired.split(",").map((skill: string) => skill.trim()),
      experienceRequired: editForm.experienceRequired,
      isActive: editForm.isActive,
    };
    
    updateJobPost(updatedJob);
    setJob(updatedJob);
    setIsEditing(false);
  ...
  const handleConnectWithApplicant = (applicantId: string) => {
    ...
    // Update UI
    setMatchingApplicants(matchingApplicants.map(applicant => 
  ...
  const hasConnectionRequest = (applicantId: string) => {
    return connections.some(
      conn => conn.applicantId === applicantId && 
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              {user?.userType === "applicant" && connectionStatus === 'none' && (
              ...
              {user?.userType === "applicant" && connectionStatus === 'pending' && (
              ...
              {user?.userType === "applicant" && connectionStatus === 'accepted' && (
                <Badge variant="outline" className="px-3 py-1 border-green-500 bg-green-50 text-green-700">
                  <CheckCircle className="h-4 w-4 mr-2" />
                  Connected
                </Badge>
              )}
              
              {user?.userType === "applicant" && connectionStatus === 'rejected' && (
                <Badge variant="outline" className="px-3 py-1 border-red-500 bg-red-50 text-red-700">
            ...
            {!isEditing ? (
                ...
                <div className="flex flex-wrap gap-4">
                  <div className="bg-secondary/10 px-4 py-2 rounded-md flex items-center">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{job.experienceRequired}+ years</p>
                    </div>
                  </div>
                  <div className="bg-secondary/10 px-4 py-2 rounded-md flex items-center">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="experienceRequired">Required Experience (years)</Label>
                    <Input
            ...
            <CardFooter className="flex-col border-t pt-6">
              <Tabs defaultValue="matching" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="matching">Matching Applicants</TabsTrigger>
                  <TabsTrigger value="connected">Connected Applicants</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="matching" className="pt-4">
                  {matchingApplicants.length === 0 ? (
                    <div className="text-center py-6">
                      ...
                      <h3 className="text-lg font-medium">No matching applicants yet</h3>
                      <p className="text-muted-foreground max-w-md mx-auto mt-1">
                        When applicants with matching skills register, they'll appear here.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                        ...
                        return (
                          <Card key={applicant.id}>
                            <CardContent className="p-4 flex justify-between items-center">
                              <div className="flex items-center space-x-3">
                                <Avatar>
                                  {applicant.profilePhoto ? (
                                    <AvatarImage src={applicant.profilePhoto} alt={applicant.fullName} />
                                  ) : (
                                    <AvatarFallback>
                                      {applicant.fullName
                                        .split(" ")
                              ...
                              <Button 
                                onClick={() => handleConnectWithApplicant(applicant.id)}
                                disabled={hasRequested}
                                variant={hasRequested ? "outline" : "default"}
                              >
                                {hasRequested ? "Connection Requested" : "Connect"}
                              </Button>
                            </CardContent>
                          </Card>
                        );
                ...
                <TabsContent value="connected" className="pt-4">
                  {connections.filter(conn => 
                    ...
                    <div className="text-center py-6">
                      <Users className="h-12 w-12 mx-auto text-muted-foreground mb-2" />
                      <h3 className="text-lg font-medium">No connected applicants yet</h3>
                      <p className="text-muted-foreground max-w-md mx-auto mt-1">
                        When applicants accept your connection requests or you accept theirs, they'll appear here.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {connections
                        ...
                        .map(connection => {
                          const applicant = getMatchingApplicants(id).find(a => a.id === connection.applicantId);
