...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const userName = user?.userType === "applicant" 
    ? applicant?.fullName || user?.email 
    : recruiter?.companyName || user?.email;
    
  const userAvatar = user?.userType === "applicant" 
    ? applicant?.profilePhoto 
    : recruiter?.profilePhoto;
    
  // Count unread notifications
  const unreadCount = notifications
    .filter(notif => notif.userId === user?.id && !notif.read)
    .length;
              ...
              <SheetContent side="right">
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex flex-col space-y-4 py-4">
                    <Link to="/dashboard" className="flex items-center space-x-2 p-2 hover:bg-gray-100 rounded">
                      <span>Dashboard</span>
                    </Link>
                    ...
                    <Link to="/notifications" className="flex items-center space-x-2 p-2 hover:bg-gray-100 rounded">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {user && (
              <>
                ...
                <Link to="/connections" className="text-gray-700 hover:text-primary">
                  Connections
                </Link>
