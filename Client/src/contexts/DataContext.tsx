...
interface DataContextType {
  ...
  // Job posts
  jobPosts: JobPost[];
  createJobPost: (jobPost: Omit<JobPost, "id" | "recruiterId" | "companyName" | "postedDate" | "isActive">) => void;
  updateJobPost: (jobPost: JobPost) => void;
  deleteJobPost: (id: string) => void;
  
  // Connections
  connections: Connection[];
  createConnection: (applicantId: string, recruiterId: string, jobPostId: string, initiatedBy: 'applicant' | 'recruiter') => void;
  ...
  // Messages
  messages: Message[];
  sendMessage: (connectionId: string, recipientId: string, content: string) => void;
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (user) {
      ...
      if (user.userType === 'applicant') {
        ...
        if (storedApplicant) {
          ...
          localStorage.setItem(`applicant-${user.id}`, JSON.stringify(newApplicant));
        }
      } else {
        ...
        if (storedRecruiter) {
          ...
          localStorage.setItem(`recruiter-${user.id}`, JSON.stringify(newRecruiter));
        }
      }
    }
  }, [user]);
  ...
  // Applicant methods
  const updateApplicant = (data: Partial<Applicant>) => {
    if (applicant) {
      const updatedApplicant = { ...applicant, ...data };
      setApplicant(updatedApplicant);
      localStorage.setItem(`applicant-${applicant.id}`, JSON.stringify(updatedApplicant));
  ...
  const addEducation = (education: Omit<Education, "id">) => {
    if (applicant) {
      const newEducation = { ...education, id: Math.random().toString(36).substring(2, 9) };
  ...
  const addSkill = (skill: Omit<Skill, "id">) => {
    if (applicant) {
      const newSkill = { ...skill, id: Math.random().toString(36).substring(2, 9) };
      const updatedApplicant = { 
        ...applicant, 
  ...
  const deleteSkill = (id: string) => {
    if (applicant) {
      ...
      toast({
        title: "Skill deleted",
        description: `${skillName || 'Skill'} has been removed from your profile`,
      });
    }
  };
  ...
  const addExperience = (experience: Omit<Experience, "id">) => {
    if (applicant) {
      const newExperience = { ...experience, id: Math.random().toString(36).substring(2, 9) };
      const updatedApplicant = { 
        ...applicant, 
  ...
  const deleteExperience = (id: string) => {
    if (applicant) {
      ...
      toast({
        ...
        description: `Your experience at ${expCompany || 'company'} has been removed`,
      });
    }
  };
  ...
  const addLanguage = (language: Omit<Language, "id">) => {
    if (applicant) {
      const newLanguage = { ...language, id: Math.random().toString(36).substring(2, 9) };
      const updatedApplicant = { 
        ...applicant, 
  ...
  const deleteLanguage = (id: string) => {
    if (applicant) {
      ...
      toast({
        ...
        description: `${langName || 'Language'} has been removed from your profile`,
      });
    }
  };
  ...
  const updateRecruiter = (data: Partial<Recruiter>) => {
    if (recruiter) {
      ...
      localStorage.setItem(`recruiter-${recruiter.id}`, JSON.stringify(updatedRecruiter));
      ...
      if (data.companyName) {
        const updatedJobPosts = jobPosts.map(job => 
          job.recruiterId === recruiter.id ? { ...job, companyName: data.companyName! } : job
        );
  ...
  // Job post methods
  const createJobPost = (jobPost: Omit<JobPost, "id" | "recruiterId" | "companyName" | "postedDate" | "isActive">) => {
    if (recruiter) {
  ...
  const deleteJobPost = (id: string) => {
    ...
    const updatedConnections = connections.filter(conn => conn.jobPostId !== id);
    setConnections(updatedConnections);
    localStorage.setItem("connections", JSON.stringify(updatedConnections));
    
    toast({
      title: "Job post deleted",
      description: `Your job post for ${jobTitle || 'position'} has been removed`,
    });
  };
  
  // Connection methods
  const createConnection = (applicantId: string, recruiterId: string, jobPostId: string, initiatedBy: 'applicant' | 'recruiter') => {
    ...
    if (initiatedBy === 'applicant') {
      notificationForRecipient = {
        ...
        message: `${applicant?.fullName || 'An applicant'} wants to connect regarding ${job?.title || 'a job post'}`,
        type: 'connection',
        read: false,
        createdAt: new Date().toISOString(),
      ...
      notificationForRecipient = {
        ...
        userId: applicantId,
        title: "New Connection Request",
        message: `${recruiter?.companyName || 'A company'} wants to connect regarding ${job?.title || 'a job post'}`,
        type: 'connection',
        read: false,
        createdAt: new Date().toISOString(),
        relatedId: newConnection.id
  ...
  const updateConnectionStatus = (id: string, status: 'accepted' | 'rejected') => {
    ...
    if (connection) {
      ...
      const applicant = allApplicants.find(a => a.id === connection.applicantId);
      const recruiter = allRecruiters.find(r => r.id === connection.recruiterId);
      
      // Create notification for initiator
      const receiverId = connection.initiatedBy === 'applicant' ? connection.applicantId : connection.recruiterId;
      ...
      const newNotification: Notification = {
        ...
        message: status === 'accepted' 
          ? `Connection for ${job?.title || 'job post'} has been accepted. You can now message each other.`
  ...
  // Message methods
  const sendMessage = (connectionId: string, recipientId: string, content: string) => {
    if (user) {
  ...
  const getConnectionsByUserId = (userId: string) => {
    return connections.filter(
      conn => (conn.applicantId === userId || conn.recruiterId === userId) && conn.status === 'accepted'
    );
  };
